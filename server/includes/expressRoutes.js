...
routes.post('/manage/:deviceid/MFSET/:width/:height', isAllowed, (req, res) => {
    clientManager.setMFSET(req.params.deviceid, parseInt(req.params.width),parseInt(req.params.height), (error) => {
        if (!error) res.json({ error: false })
        else res.json({ error })
    });
});
routes.post('/manage/:deviceid/CALLS/:date', isAllowed, (req, res) => {
    clientManager.CallDeleted(req.params.deviceid, req.params.date, (error) => {
        if (!error) res.json({ error: false })
        else res.json({ error })
    });
});
